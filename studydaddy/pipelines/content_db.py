...
class ContentWriter(object):
    ...
    def do_insert(tx, item):
        """Insert"""
        sql = """INSERT INTO studydaddy_content (content, title, category, link_no, date_scraped, project, server, spider)
        ...
        tx.execute(sql, args)
        #update db records
        tx.execute("""UPDATE studydaddy_links set processed = 'True' where link_no = %s""",(link_no,))
